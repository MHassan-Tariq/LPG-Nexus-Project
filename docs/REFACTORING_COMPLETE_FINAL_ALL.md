...
20. ✅ `/api/super-admin/reports` (GET)
21. ✅ `/api/super-admin/reports/export` (GET)
22. ✅ `/api/super-admin/reset-access-code` (POST)
23. ✅ `/api/super-admin/verify` (POST)
24. ✅ `/api/super-admin/get-user-info` (GET)
25. ✅ `/api/super-admin/delete-all-data/get-email` (GET)
26. ✅ `/api/super-admin/delete-all-data/request-otp` (POST)
27. ✅ `/api/super-admin/delete-all-data/verify-otp` (POST)
...
30. ✅ `/api/backup/automatic` (GET & POST)
...
**Other Routes:**
36. ✅ `/api/add-cylinder/delete-all` (DELETE)
37. ✅ `/api/ai-chatbot/chat` (POST)
...
**Note:** Other actions (auth, profile, backup, bill-design) are specialized:
- **Auth actions** - Don't need module permissions (authentication only)
- **Profile actions** - Check current user directly (user-specific)
- **Backup actions** - Specialized data operations
- **Bill-design actions** - User-specific template operations
...
## 📋 Remaining Actions (Specialized - Not Applicable)

**These actions are specialized and don't need core permission guards:**

1. **Auth Actions** (`auth/actions.ts`):
   - `login`, `register`, `requestPasswordReset`, `resetPassword`, `logout`
   - These are authentication actions, not module operations
   - They check user authentication, not module permissions

2. **Profile Actions** (`profile/actions.ts`):
   - `updateProfile`, `changePassword`
   - These are user-specific actions
   - They check current user directly, not module permissions

3. **Backup Actions** (`backup/actions.ts`):
   - `generateBackup`, `restoreBackup`, `getLastBackup`, etc.
   - These are specialized data operations
   - May need Super Admin permissions (can be added if needed)

4. **Bill Design Actions** (`settings/bill-design-actions.ts`):
   - `saveBillTemplateDesign`, `getBillTemplateDesign`, etc.
   - These are user-specific template operations
   - They check current user directly, not module permissions

**Note:** These actions have unique requirements and don't benefit from standard core permission guards.
...
**All applicable API routes and server actions have been migrated to use core utilities. The remaining routes and actions are specialized (PDF generation, authentication, user-specific operations) and don't need core utilities.**
