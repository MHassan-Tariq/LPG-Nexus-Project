...
interface CylinderBillRendererProps {
  design: any;
  entry: {
    id: string;
...
export function CylinderBillRenderer({ design, entry, logo = null, barcode = null, qrCode = null }: CylinderBillRendererProps) {
  if (!design) {
    return null;
  }
        ...
        <div className="grid grid-cols-2 gap-8 mb-6">
          <div>
            <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">ID: </span>{entry.customerId || (entry.customerName.includes(' · ') ? entry.customerName.split(' · ')[0] : "4")}
            </p>
            <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">Customer Name: </span>{entry.customerName.includes(' · ') ? entry.customerName.split(' · ')[1] : entry.customerName}
            </p>
            {entry.customerAddress && (
              <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
                <span className="font-semibold">Address: </span>{entry.customerAddress}
              </p>
            )}
            {entry.customerPhone && (
              <p style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
                <span className="font-semibold">Phone: </span>{entry.customerPhone}
              </p>
            )}
          </div>
          <div>
            {entry.cylinderType === "DELIVERED" && entry.deliveredBy && (
              <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
                <span className="font-semibold">Delivered By: </span>{entry.deliveredBy}
              </p>
            )}
            <p style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">Date: </span>{format(entry.deliveryDate, "d MMMM yyyy")}
            </p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full" style={{ borderWidth: `${design.tableBorderThickness || 1}px` }}>
            ...
            <tbody>
              <tr style={{ backgroundColor: rowBg }}>
                ...
                {design.showPrices && entry.cylinderType === "DELIVERED" && (
                  <>
                    <td className={cn(
                      "px-4 py-3 border",
                      design.tableAlignment === "left" && "text-left",
                      ...
                      <span className="whitespace-nowrap">{formatCurrency(entry.unitPrice)}</span>
                    </td>
                    <td className={cn(
                      ...
                      <span className="whitespace-nowrap">{formatCurrency(entry.amount)}</span>
                    </td>
                  </>
                )}
                {entry.cylinderType === "RECEIVED" && (
                  <>
                    ...
                    <td className={cn(
                      ...
                      {entry.paymentAmount && entry.paymentAmount > 0
                        ? <span className="whitespace-nowrap">{formatCurrency(entry.paymentAmount)}</span>
                        : "—"}
                    </td>
                  </>
                )}
          ...
          <div className="flex justify-end">
            <div className="w-64 space-y-2">
              ...
              <div className="flex justify-between" style={{ color: textColor, fontSize: `${fontSize}px` }}>
               ...
               <div className="flex justify-between" style={{ color: textColor, fontSize: `${fontSize}px` }}>
                 <span>Total:</span>
                 <span>{entry.quantity} × {Number(entry.unitPrice || 0).toLocaleString('en-US')}</span>
               </div>
              <div
                className="h-px my-2"
      ...
      {entry.cylinderType === "RECEIVED" && (
        <div className="mb-6">
          <div className="flex justify-end">
            <div className="w-64 space-y-2">
              <div className="flex justify-between" style={{ color: textColor, fontSize: `${fontSize}px` }}>
                <span>Empty Cylinders Received:</span>
                <span>{entry.emptyCylinderReceived ?? entry.quantity ?? 0}</span>
              </div>
              {entry.paymentType && entry.paymentType !== "NONE" && (
                <>
                  <div className="flex justify-between" style={{ color: textColor, fontSize: `${fontSize}px` }}>
                    <span>Payment Type:</span>
                    <span>{entry.paymentType.charAt(0) + entry.paymentType.slice(1).toLowerCase()}</span>
                  </div>
                  {entry.paymentAmount && entry.paymentAmount > 0 && (
                    <>
                      <div className="flex justify-between" style={{ color: textColor, fontSize: `${fontSize}px` }}>
                      ...
                      {entry.paymentReceivedBy && (
                        <div className="flex justify-between" style={{ color: textColor, fontSize: `${fontSize}px` }}>
                          ...
                          <span>{entry.paymentReceivedBy}</span>
                        </div>
                      )}
                    </>
                  )}
