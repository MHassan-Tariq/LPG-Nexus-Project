...
const billDesignSchema = z.object({
  ...
  theme: z.enum(["light", "dark", "colored", "minimal", "classic", "bold", "modern", "rose"]).default("light"),
  ...
  headerTitle: z.string().default("INVOICE"),
...
type BillDesignFormValues = z.infer<typeof billDesignSchema>;
...
export function BillDesigningTab() {
  ...
  const form = useForm<BillDesignFormValues>({
    resolver: zodResolver(billDesignSchema),
    defaultValues: {
      pageSize: "A4",
    ...
    async function loadSavedDesign() {
      ...
      try {
        const [designResult, presetsResult] = await Promise.all([
          getBillTemplateDesign(),
          getBillPresets(),
        ]);
        
        if (designResult.success && designResult.data) {
          const saved = designResult.data as any;
          form.reset(saved);
          if (saved.customLogo) {
  ...
  async function handleSavePreset() {
    ...
    try {
      ...
      const result = await saveBillPreset(presetName.trim(), currentValues);
      ...
      if (result.success) {
        ...
        const presetsResult = await getBillPresets();
        if (presetsResult.success && presetsResult.data) {
          setPresets(presetsResult.data);
  ...
  function handleLoadPreset(preset: any) {
    ...
    if (preset.data.customLogo && typeof preset.data.customLogo === "string") {
      setLogoPreview(preset.data.customLogo);
    } else {
      setLogoPreview(null);
    }
    if (preset.data.customBarcode && typeof preset.data.customBarcode === "string") {
      setBarcodePreview(preset.data.customBarcode);
    } else {
      setBarcodePreview(null);
    }
    if (preset.data.customQRCode && typeof preset.data.customQRCode === "string") {
      setQrCodePreview(preset.data.customQRCode);
    } else {
  ...
  async function handleDeletePreset(presetId: string, presetName: string) {
    if (confirm(`Are you sure you want to delete preset "${presetName}"?`)) {
      try {
        const result = await deleteBillPreset(presetId);
        if (result.success) {
          setToast({
  ...
  async function onSubmit(data: BillDesignFormValues) {
    setIsSubmitting(true);
    try {
      // Get the latest form values to ensure we have all updates
      const currentValues = form.getValues();
      const templateData = {
        ...currentValues,
        customLogo: logoPreview || currentValues.customLogo,
        customBarcode: barcodePreview || currentValues.customBarcode,
        customQRCode: qrCodePreview || currentValues.customQRCode,
      };

      const result = await saveBillTemplateDesign(templateData);

      if (result.success) {
        setToast({ message: "Your custom bill design has been saved successfully!", type: "success" });
        setHasUnsavedChanges(false);
        
        // Check if there's a returnUrl to redirect back to the print bill page
        const returnUrl = searchParams.get("returnUrl");
        if (returnUrl) {
          // Decode the returnUrl and redirect immediately
          const decodedUrl = decodeURIComponent(returnUrl);
          // Use window.location for a full page reload to ensure design is refreshed
          setTimeout(() => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-[#e5eaf4] bg-white p-6">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {showPresetsSection ? "Hide Presets" : "Save Preset"}
            </Button>
            <Button
              type="button"
              variant="outline"
            ...
            <Button
              ...
              className="gap-2 rounded-xl bg-[#2544d6] hover:bg-[#1e3fb8] disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {/* Presets Section */}
        {showPresetsSection && (
          <div className="rounded-xl border border-[#e5eaf4] bg-white p-6 mt-6">
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-slate-900 flex items-center gap-2 mb-1">
                <Bookmark className="h-5 w-5 text-[#2544d6]" />
                Save Preset
              </h3>
              <p className="text-sm text-slate-500">Save your current design as a preset for quick access later</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="text-base font-semibold text-slate-900 flex items-center gap-2 mb-1">
                      <Bookmark className="h-4 w-4 text-[#2544d6]" />
                      Saved Presets
                    </h4>
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="text-base font-semibold text-slate-900 flex items-center gap-2 mb-1">
                      <Clock className="h-4 w-4 text-[#2544d6]" />
            ...
            <div className="space-y-2">
              {themes.map((theme) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    form.setValue("theme", theme.id as any, { shouldDirty: true, shouldValidate: true });
                    form.setValue("primaryColor", theme.colors[2], { shouldDirty: true, shouldValidate: true });
                    form.setValue("backgroundColor", theme.colors[0], { shouldDirty: true, shouldValidate: true });
                    // Update header color to match theme
                    form.setValue("headerColor", theme.colors[2], { shouldDirty: true, shouldValidate: true });
                    // Update table header color and text color based on theme
                    form.setValue("tableHeaderBgColor", theme.colors[2], { shouldDirty: true, shouldValidate: true });
                    // For dark theme, use white text, otherwise use theme's text color
                    if (theme.id === "dark") {
                      form.setValue("tableTextColor", "#ffffff", { shouldDirty: true, shouldValidate: true });
                    } else {
                      form.setValue("tableTextColor", theme.colors[1] === "#ffffff" ? "#000000" : theme.colors[1], { shouldDirty: true, shouldValidate: true });
                    }
                    setHasUnsavedChanges(true);
                  }}
                  className={cn(
            ...
            >
              <div className="flex justify-center w-full overflow-x-auto">
                <CylinderBillRenderer
                  ...
                  entry={{
                    id: "sample-id",
                    billCreatedBy: "Sample Creator",
                    cylinderType: "DELIVERED" as const,
                    cylinderLabel: "45kg (Commercial cylinder)",
                    deliveredBy: "Imran",
                    quantity: 5,
                    unitPrice: 60000,
                    amount: 300000,
                    customerName: "4 · Arham",
                    customerId: "4",
                    verified: false,
                    description: null,
                    deliveryDate: new Date("2025-12-14"),
                    createdAt: new Date(),
                    customerAddress: "Kohinoor Plaza",
                    customerPhone: "8939724422",
                  }}
                  logo={logoPreview}
        ...
        <div className="grid grid-cols-2 gap-8 mb-6">
          <div>
            <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">ID:</span> 4
            </p>
            <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">Customer Name:</span> Arham
            </p>
          </div>
          <div>
            <p className="mb-1" style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">Delivered By:</span> Imran
            </p>
            <p style={{ color: bodyTextColor, fontSize: `${fontSize}px` }}>
              <span className="font-semibold">Date:</span> December 14, 2025
            </p>
          </div>
        </div>
      </div>
